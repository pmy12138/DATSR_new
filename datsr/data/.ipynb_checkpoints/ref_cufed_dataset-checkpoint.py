...
class RefCUFEDDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if self.opt.get('use_ColorJitter', False):
    ...
    def __getitem__(self, index):
        ...
        # --- add Gaussian noise to LR input if configured ---
        noise_sigma = self.opt.get('noise_sigma', 0)
        if noise_sigma > 0:
            img_in_lq_clean = img_in_lq.clone()
            noise = torch.randn_like(img_in_lq) * (noise_sigma / 255.0)
            img_in_lq = torch.clamp(img_in_lq + noise, 0, 1)
        else:
            img_in_lq_clean = img_in_lq
