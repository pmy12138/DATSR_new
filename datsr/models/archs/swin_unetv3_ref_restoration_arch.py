...
class WaveletDynamicAggregationRestoration(DynamicAggregationRestoration):
    """
    Extended DynamicAggregationRestoration with WaveAttentionBlock appended
    after each SwinBlock in the U-Net body (Scheme 2 integration).
    """

    def __init__(self, ngf=64, n_blocks=16, groups=8, img_size=40,
                 patch_size=1, in_chans=3, embed_dim=64,
                 depths=(6, 6, 6, 6), num_heads=(6, 6, 6, 6),
                 window_size=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False,
                 wave_num_heads=4, wave_ffn_expansion=2.):
        super().__init__(
            ngf=ngf, n_blocks=n_blocks, groups=groups, img_size=img_size,
        ...
        from datsr.models.archs.wavelet_utils_arch import WaveAttentionBlock

        self.wave_down_large = WaveAttentionBlock(
            ngf, wave_num_heads, wave_ffn_expansion)
        self.wave_down_medium = WaveAttentionBlock(
            ngf, wave_num_heads, wave_ffn_expansion)
        self.wave_up_small = WaveAttentionBlock(
            ngf, wave_num_heads, wave_ffn_expansion)
        self.wave_up_medium = WaveAttentionBlock(
            ngf, wave_num_heads, wave_ffn_expansion)
        self.wave_up_large = WaveAttentionBlock(
            ngf, wave_num_heads, wave_ffn_expansion)
    ...
    def forward(self, base, x, pre_offset_flow_sim, img_ref_feat):
        ...
        pre_relu1_swapped_feat = self.flow_warp(
            img_ref_feat['relu1_1'], pre_flow['relu1_1'])
        pre_relu2_swapped_feat = self.flow_warp(
            img_ref_feat['relu2_1'], pre_flow['relu2_1'])
        pre_relu3_swapped_feat = self.flow_warp(
            img_ref_feat['relu3_1'], pre_flow['relu3_1'])

        # Unet
        x0 = self.unet_head(base)

        # -------------- Down ------------------
        # large scale
        down_relu1_offset = torch.cat(
            [x0, pre_relu1_swapped_feat, img_ref_feat['relu1_1']], 1)
        down_relu1_offset = self.lrelu(
            self.down_large_offset_conv1(down_relu1_offset))
        down_relu1_offset = self.lrelu(
            self.down_large_offset_conv2(down_relu1_offset))
        down_relu1_swapped_feat = self.lrelu(
        ...
        h = self.down_head_large(h)
        h = self.down_body_large(h) + x0
        h = self.wave_down_large(h)          # wavelet attention
        x1 = self.down_tail_large(h)

        # medium scale
        down_relu2_offset = torch.cat(
            [x1, pre_relu2_swapped_feat, img_ref_feat['relu2_1']], 1)
        down_relu2_offset = self.lrelu(
            self.down_medium_offset_conv1(down_relu2_offset))
        down_relu2_offset = self.lrelu(
            self.down_medium_offset_conv2(down_relu2_offset))
        down_relu2_swapped_feat = self.lrelu(
        ...
        h = self.down_head_medium(h)
        h = self.down_body_medium(h) + x1
        h = self.wave_down_medium(h)         # wavelet attention
        x2 = self.down_tail_medium(h)

        # -------------- Up ------------------
        # dynamic aggregation for relu3_1 reference feature
        relu3_offset = torch.cat(
            [x2, pre_relu3_swapped_feat, img_ref_feat['relu3_1']], 1)
        relu3_offset = self.lrelu(self.up_small_offset_conv1(relu3_offset))
        relu3_offset = self.lrelu(self.up_small_offset_conv2(relu3_offset))
        relu3_swapped_feat = self.lrelu(
            self.up_small_dyn_agg(
                [img_ref_feat['relu3_1'], relu3_offset],
                pre_offset['relu3_1'], pre_similarity['relu3_1']))

        # small scale
        h = torch.cat([x2, relu3_swapped_feat], 1)
        h = self.up_head_small(h)
        h = self.up_body_small(h) + x2
        h = self.wave_up_small(h)            # wavelet attention
        x = self.up_tail_small(h)

        # dynamic aggregation for relu2_1 reference feature
        relu2_offset = torch.cat(
            [x, pre_relu2_swapped_feat, img_ref_feat['relu2_1']], 1)
        relu2_offset = self.lrelu(self.up_medium_offset_conv1(relu2_offset))
        relu2_offset = self.lrelu(self.up_medium_offset_conv2(relu2_offset))
        relu2_swapped_feat = self.lrelu(
        ...
        h = self.up_head_medium(h)
        h = self.up_body_medium(h) + x
        h = self.wave_up_medium(h)           # wavelet attention
        x = self.up_tail_medium(h)

        # dynamic aggregation for relu1_1 reference feature
        relu1_offset = torch.cat(
            [x, pre_relu1_swapped_feat, img_ref_feat['relu1_1']], 1)
        relu1_offset = self.lrelu(self.up_large_offset_conv1(relu1_offset))
        relu1_offset = self.lrelu(self.up_large_offset_conv2(relu1_offset))
        relu1_swapped_feat = self.lrelu(
        ...
        h = self.up_head_large(h)
        h = self.up_body_large(h) + x
        h = self.wave_up_large(h)            # wavelet attention
        x = self.up_tail_large(h)

        return x


class WaveletSwinUnetv3RestorationNet(nn.Module):
    """
    Wavelet-enhanced restoration network combining:
    - Scheme 1: WaveletDenoiseModule (WDM) at the input stage
    - Scheme 2: WaveAttentionBlock appended in U-Net body

    The WDM denoises the noisy LR before content extraction and SR.
    The network returns (sr_output, denoised_lq) so that denoised_lq
    can be supervised during training.
    """
    ...
    def __init__(self, ngf=64, n_blocks=16, groups=8, embed_dim=64,
                 ...
                 wave_num_heads=4, wave_ffn_expansion=2.):
        super().__init__()
        from datsr.models.archs.wavelet_utils_arch import WaveletDenoiseModule

        self.wdm = WaveletDenoiseModule(in_channels=3, shared_denoiser=False)

        self.content_extractor = ContentExtractor(
            in_nc=3, out_nc=3, nf=ngf, n_blocks=n_blocks)

        self.dyn_agg_restore = WaveletDynamicAggregationRestoration(
            ngf=ngf, n_blocks=n_blocks, groups=groups,
            embed_dim=ngf, depths=depths, num_heads=num_heads,
            window_size=window_size, use_checkpoint=use_checkpoint,
            wave_num_heads=wave_num_heads,
            wave_ffn_expansion=wave_ffn_expansion)
    ...
    def forward(self, x, pre_offset_flow_sim, img_ref_feat):
        """
        Args:
            x: noisy LR input (B, 3, H, W).
        Returns:
            sr_output: super-resolved image (B, 3, 4H, 4W).
            denoised_lq: wavelet-denoised LR (B, 3, H, W).
        """
        denoised_lq = self.wdm(x)

        base = F.interpolate(denoised_lq, None, 4, 'bilinear', False)
        content_feat = self.content_extractor(denoised_lq)

        upscale_restore = self.dyn_agg_restore(
            base, content_feat, pre_offset_flow_sim, img_ref_feat)

        return upscale_restore + base, denoised_lq