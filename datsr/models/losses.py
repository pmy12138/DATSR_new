...
class FFTLoss(nn.Module):
    """Frequency-domain L1 loss computed via FFT."""
...
class WaveletLoss(nn.Module):
    """
    Loss computed in wavelet (Haar) domain.
    Decomposes both pred and target via Haar DWT, then applies L1 loss
    on each sub-band with configurable weights.
    """

    def __init__(self, loss_weight=1.0, ll_weight=10.0, hf_weight=1.0,
                 reduction='mean'):
        super().__init__()
        from datsr.models.archs.wavelet_utils_arch import DWT_2D
    ...
    def forward(self, pred, target, **kwargs):
        C = pred.shape[1]
        pred_w = self.dwt(pred)
        target_w = self.dwt(target)

        ll_pred = pred_w[:, 0 * C:1 * C]
        lh_pred = pred_w[:, 1 * C:2 * C]
        hl_pred = pred_w[:, 2 * C:3 * C]
        hh_pred = pred_w[:, 3 * C:4 * C]

        ll_tgt = target_w[:, 0 * C:1 * C]
        lh_tgt = target_w[:, 1 * C:2 * C]
        hl_tgt = target_w[:, 2 * C:3 * C]
        hh_tgt = target_w[:, 3 * C:4 * C]

        loss_ll = self._loss(ll_pred, ll_tgt)
        loss_lh = self._loss(lh_pred, lh_tgt)
        loss_hl = self._loss(hl_pred, hl_tgt)
        loss_hh = self._loss(hh_pred, hh_tgt)

        loss = (self.ll_weight * loss_ll +
                self.hf_weight * (loss_lh + loss_hl + loss_hh))
        return self.loss_weight * loss


class MaskSparseLoss(nn.Module):
    """L1 sparsity loss for wavelet-denoising masks."""

    def __init__(self, loss_weight=1.0):
        super(MaskSparseLoss, self).__init__()
        self.loss_weight = loss_weight

    def forward(self, mask_dict):
        """
        Args:
            mask_dict: dict containing masks from different subbands
                e.g., {'lh': mask_lh, 'hl': mask_hl, 'hh': mask_hh}
        Returns:
            sparse_loss: L1 norm of masks
        """
        if not mask_dict:
